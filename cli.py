...
@main.command()
@click.option('--ip', required = True, help = "Server's ip address")
@click.option('--port', '-p', type = int, required = True, help = "Server's port")
@click.option('--consistency', '-c', type = click.Choice(['linearizability', 'eventual']), required = True, default = 'linearizability', help = "Type of consistency")
...
@click.option('--ip', required = True, help = "Server's ip address")
@click.option('--port', '-p', type = int, required = True, help = "Server's port")
@click.option('--k', '-k', type = int, required = True, help = "Replication factor")
@click.argument('key',  type = str, required = True, nargs = -1)
def delete(ip, port, k, key):
	'''
	...
	if key.isnumeric():
		...
		server_id = ip + ":" + str(port)
		starter_hash = sha1(server_id.encode('utf-8')).hexdigest()
		data = 'delete' + ":" + key + ":" + str(k) + ":" + starter_hash
		client = Client(ip,port)
...
def depart(ip, port):															# Einai etoimh
	...
	'''
	if port == 5001:
		click.echo("Error: Invalid value for --port")
	else:
		client = Client(ip,port)
